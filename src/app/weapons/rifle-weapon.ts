...
export class RifleWeapon {
  static build(): THREE.Group {
    ...
    // ── Upper receiver ──────────────────────────────────────────────────────────
    g.add(box(0.038, 0.044, 0.22, matReceiver, 0, 0.01, -0.002));
    g.add(box(0.022, 0.009, 0.222, matMetal, 0, 0.04, -0.002));
    for (let i = 0; i < 11; i++)
      g.add(box(0.02, 0.004, 0.01, matReceiver, 0, 0.047, -0.108 + i * 0.022));
    ...
    g.add(box(0.004, 0.024, 0.062, matMetal, 0.021, 0.006, -0.03));
    g.add(cyl(0.003, 0.003, 0.056, 8, matBarrel, 0.022, 0.022, -0.005, Math.PI / 2));
    g.add(box(0.006, 0.02, 0.01, matMetal, 0.021, 0.012, 0.002));
    g.add(cyl(0.005, 0.005, 0.044, 12, matMetal, 0, 0.0, 0.102, 0, 0, Math.PI / 2));
    g.add(cyl(0.005, 0.005, 0.044, 12, matMetal, 0, 0.0, -0.098, 0, 0, Math.PI / 2));
    g.add(box(0.038, 0.008, 0.01, matMetal, 0, 0.04, 0.112));

    // ── Lower receiver ──────────────────────────────────────────────────────────
    g.add(box(0.034, 0.036, 0.2, matReceiver, 0, -0.016, 0.002));
    const magWell = box(0.03, 0.03, 0.072, matReceiver, 0, -0.038, -0.006);
    ...
    g.add(box(0.01, 0.008, 0.01, matReceiver, 0, -0.006, 0.118));
    ...
    for (let i = 0; i < 6; i++) {
      ...
      const cn = new THREE.Mesh(new THREE.BoxGeometry(0.007, 0.012, 0.02), matReceiver);
      cn.position.set(Math.cos(a) * 0.02, 0.022 + Math.sin(a) * 0.02, -0.092);
      cn.rotation.z = a;
      g.add(cn);
    }
    ...
    // Gas tube
    g.add(cyl(0.003, 0.003, 0.078, 8, matBarrel, 0, 0.04, -0.116, Math.PI / 2));
    g.add(cyl(0.006, 0.006, 0.012, 10, matMetal, 0, 0.036, -0.08, Math.PI / 2));
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const slot = new THREE.Mesh(new THREE.BoxGeometry(0.005, 0.02, 0.026), matReceiver);
      slot.position.set(Math.sin(a) * 0.013, 0.022 + Math.cos(a) * 0.013, -0.282);
      slot.rotation.z = a;
      g.add(slot);
    }
    g.add(box(0.005, 0.01, 0.028, matMuzzle, 0, 0.009, -0.282));
    ...
    g.add(cyl(0.026, 0.026, 0.008, 20, matMetal, 0, 0.01, hgZ - hgLen / 2 + 0.004, Math.PI / 2));
    g.add(cyl(0.026, 0.026, 0.008, 20, matMetal, 0, 0.01, hgZ + hgLen / 2 - 0.004, Math.PI / 2));
    ...
    peq.position.set(0.0, 0.062, -0.038);
    g.add(peq);

    // ── SureFire M600C Scout Light ───────────────────────────────────────────────
    const sf = buildScoutLight();
    sf.position.set(-0.04, 0.022, -0.148);
    ...
    g.add(cyl(0.003, 0.003, 0.01, 8, matMetal, 0, 0.024, 0.086, Math.PI / 2));
    ...
    const matMagDark = new THREE.MeshStandardMaterial({ color: 0x0a0a0a, roughness: 0.9 });
    mag.add(box(0.03, 0.096, 0.044, matMagBody));
    mag.add(box(0.028, 0.092, 0.01, matMagDark, 0, 0, -0.027));
    mag.add(box(0.028, 0.09, 0.006, matMagDark, 0, 0, 0.025));
    mag.add(
      box(
        0.005,
        0.042,
    ...
    mag.add(box(0.034, 0.01, 0.048, matMetal, 0, -0.053, 0));
    mag.add(box(0.008, 0.008, 0.01, matMagDark, 0.016, -0.058, 0));
    for (let i = 0; i < 3; i++)
    ...
    grip.add(box(0.03, 0.082, 0.046, matGripBlack));
    grip.add(
      box(
        0.012,
        0.074,
    ...
    // Trigger
    g.add(box(0.01, 0.022, 0.008, matMetal, 0, -0.042, 0.078));
    ...
    g.add(cyl(0.0146, 0.0146, 0.15, 20, matReceiver, 0, 0.012, 0.208, Math.PI / 2));
    for (let i = 0; i < 6; i++)
      g.add(cyl(0.015, 0.015, 0.005, 20, matMetal, 0, 0.001, 0.14 + i * 0.023, Math.PI / 2));
    g.add(box(0.038, 0.042, 0.008, matMetal, 0, 0.008, 0.136));
    g.add(cyl(0.019, 0.019, 0.01, 18, matMetal, 0, 0.012, 0.144, Math.PI / 2));
    for (let cs = 0; cs < 3; cs++) {
      const a = (cs / 3) * Math.PI * 2;
      const cn = new THREE.Mesh(new THREE.BoxGeometry(0.006, 0.012, 0.012), matReceiver);
      cn.position.set(Math.cos(a) * 0.02, 0.012 + Math.sin(a) * 0.02, 0.144);
      g.add(cn);
    }
    // Stock body
    g.add(box(0.028, 0.052, 0.12, matGripBlack, 0, 0.006, 0.242));
    g.add(box(0.026, 0.012, 0.098, matGripBlack, 0, 0.034, 0.23));
    g.add(box(0.026, 0.022, 0.05, matGripBlack, 0, 0.03, 0.258));
    // Sling loop
    g.add(box(0.008, 0.026, 0.012, matMetal, 0, -0.02, 0.294));
    g.add(cyl(0.004, 0.004, 0.024, 8, matMetal, 0, -0.01, 0.294, 0));
    // QD socket
    g.add(cyl(0.006, 0.006, 0.01, 12, matMetal, 0.016, 0.006, 0.262, 0, 0, Math.PI / 2));
    // Rubber buttpad
    g.add(box(0.03, 0.058, 0.014, matACOGRubber, 0, 0.006, 0.305));
    for (let r = 0; r < 3; r++)
      g.add(box(0.03, 0.004, 0.014, matMetal, 0, -0.014 + r * 0.016, 0.31));
    // Release lever
    g.add(box(0.014, 0.01, 0.018, matGripBlack, 0, 0.034, 0.194));
    ...
    g.add(cyl(0.003, 0.003, 0.024, 8, matMetal, 0, mbusRailY + 0.01, mbusRearZ, 0, 0, Math.PI / 2));
    g.add(
      box(
        0.018,
        0.024,
    ...
    g.add(cyl(0.003, 0.003, 0.03, 8, matMetal, -0.015, mbusFrontY - 0.002, mbusFrontZ, 0));
    g.add(
      box(
        0.014,
        0.014,
    ...
    g.add(
      cyl(0.003, 0.003, 0.018, 8, matMetal, 0, mbusFrontY + 0.011, mbusFrontZ, 0, 0, Math.PI / 2),
    );
    g.add(
      box(
        0.004,
