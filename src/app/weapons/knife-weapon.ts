...
import {
  _knifeBladeTex, _knifeBladeNrm,
  _leatherTex, _leatherNrm,
...
export class KnifeWeapon {
  ...
  static build(): THREE.Group {
    ...
    const matBlade = new THREE.MeshStandardMaterial({
      map: _knifeBladeTex, normalMap: _knifeBladeNrm,
      normalScale: new THREE.Vector2(0.9, 0.9),
      color: 0x1a1a1a, roughness: 0.20, metalness: 0.94,
    });
    const matEdge = new THREE.MeshStandardMaterial({
      map: _knifeBladeTex, normalMap: _knifeBladeNrm,
      normalScale: new THREE.Vector2(0.5, 0.5),
      color: 0xb8b4a8, roughness: 0.06, metalness: 0.98,
    });
    const matFuller = new THREE.MeshStandardMaterial({
      map: _knifeBladeTex, normalMap: _knifeBladeNrm,
      normalScale: new THREE.Vector2(1.2, 1.2),
      color: 0x080808, roughness: 0.24, metalness: 0.90,
    });
    const matGuard = new THREE.MeshStandardMaterial({
      map: _metalTex, normalMap: _metalNorm,
      normalScale: new THREE.Vector2(0.7, 0.7),
      color: 0x111111, roughness: 0.42, metalness: 0.92,
    });
    const matGrip = new THREE.MeshStandardMaterial({
      map: _leatherTex, normalMap: _leatherNrm,
      normalScale: new THREE.Vector2(1.8, 1.8),
      color: 0x1a0e04, roughness: 0.96, metalness: 0.01,
    });
    const matGripDark = new THREE.MeshStandardMaterial({
      map: _leatherTex, normalMap: _leatherNrm,
      normalScale: new THREE.Vector2(1.8, 1.8),
      color: 0x0c0704, roughness: 0.99, metalness: 0.0,
    });
    const matPommel = new THREE.MeshStandardMaterial({
      map: _metalTex, normalMap: _metalNorm,
      normalScale: new THREE.Vector2(0.8, 0.8),
      color: 0x1c1c1c, roughness: 0.36, metalness: 0.90,
    });

    // Blade (ExtrudeGeometry — clip-point silhouette)
    const bladeShape = new THREE.Shape();
    bladeShape.moveTo(0, 0);
    bladeShape.bezierCurveTo(-0.050, -0.004, -0.130, -0.003, -0.172, 0.003);
    bladeShape.quadraticCurveTo(-0.192, 0.014, -0.202, 0.008);
    bladeShape.lineTo(-0.160, 0.023);
    ...
    bevelShape.bezierCurveTo(-0.050, -0.004, -0.130, -0.003, -0.172, 0.003);
    ...
    // Guard
    g.add(box(0.013, 0.042, 0.015, matGuard, 0, 0.003, 0.020));
    const quU = new THREE.Mesh(new THREE.CylinderGeometry(0.005, 0.009, 0.020, 14), matGuard);
    ...
    g.add(quL);
    g.add(box(0.014, 0.042, 0.002,
      new THREE.MeshStandardMaterial({ color: 0x0d0d0d, roughness: 0.50, metalness: 0.86 }),
      0, 0.003, 0.012));
    g.add(cyl(0.014, 0.014, 0.008, 14,
      new THREE.MeshStandardMaterial({ color: 0x7a6828, roughness: 0.32, metalness: 0.80 }),
      0, 0.002, 0.033, Math.PI / 2));

    // Grip — 22 leather washers
    const GRIP_START = 0.040;
    ...
    for (let i = 0; i < 22; i++) {
      ...
      const washer = new THREE.Mesh(new THREE.CylinderGeometry(r, r, 0.0050, 12), mat);
    ...
    const pomCollar = new THREE.Mesh(new THREE.CylinderGeometry(0.016, 0.018, 0.008, 14), matPommel);
    ...
    pomBody.position.set(0, 0.002, POM_Z + 0.020);
    g.add(pomBody);
    ...
    g.add(pomCap);
    g.add(cyl(0.004, 0.004, 0.003, 12,
      new THREE.MeshStandardMaterial({ color: 0x040404, roughness: 1.0 }),
      0, 0.008, POM_Z + 0.043, Math.PI / 2));
