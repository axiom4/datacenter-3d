...
export class HandsWeapon {
  ...
  static build(): THREE.Group {
    ...
    function buildHand(side: 1 | -1): THREE.Group {
      ...
      const wrist = new THREE.Mesh(new THREE.CylinderGeometry(0.032, 0.038, 0.08, 12), matSkin);
      wrist.rotation.x = Math.PI / 2;
      wrist.position.set(0, 0, 0.072);
      h.add(wrist);

      const forearm = new THREE.Mesh(new THREE.CylinderGeometry(0.036, 0.044, 0.17, 12), matSkin);
      forearm.rotation.x = Math.PI / 2;
      forearm.position.set(0, -0.004, 0.162);
      h.add(forearm);

      const sleeve = new THREE.Mesh(new THREE.CylinderGeometry(0.044, 0.048, 0.03, 12), matSleeve);
      sleeve.rotation.x = Math.PI / 2;
      sleeve.position.set(0, -0.002, 0.25);
      h.add(sleeve);

      // Elbow — sphere at the bend, pops into view during hook
      const elbow = new THREE.Mesh(new THREE.SphereGeometry(0.04, 10, 8), matSkin);
      elbow.position.set(0, -0.006, 0.272);
      h.add(elbow);

      // Upper arm — extends back behind elbow; swings into view when arm rotates for hook
      const upperArm = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.044, 0.16, 12), matSkin);
      ...
      const upperSleeve = new THREE.Mesh(
        new THREE.CylinderGeometry(0.046, 0.05, 0.162, 12),
        matSleeve,
      );
      ...
      openGroup.add(box(0.08, 0.016, 0.088, matSkin, 0, 0, 0));
      openGroup.add(box(0.078, 0.004, 0.086, matSkinDark, 0, 0.01, 0));
      for (let v = 0; v < 3; v++)
        openGroup.add(box(0.004, 0.003, 0.06, matVein, (-0.02 + v * 0.02) * s, 0.013, -0.01));
      ...
      const fingerDefs: {
        ...
        { ox: -0.029, len: [0.03, 0.024, 0.018], r: [0.012, 0.01, 0.008] },
        { ox: -0.009, len: [0.034, 0.026, 0.02], r: [0.012, 0.01, 0.008] },
        { ox: 0.011, len: [0.032, 0.024, 0.018], r: [0.011, 0.009, 0.007] },
        { ox: 0.03, len: [0.024, 0.018, 0.014], r: [0.009, 0.008, 0.007] },
      ];
      for (const fd of fingerDefs) {
        let zOff = -0.044;
        fd.len.forEach((l, i) => {
          const seg = new THREE.Mesh(
            new THREE.CylinderGeometry(fd.r[i] * 0.9, fd.r[i], l, 8),
            matSkin,
          );
      ...
      const thumbBase = new THREE.Mesh(new THREE.CylinderGeometry(0.011, 0.013, 0.03, 8), matSkin);
      ...
      thumbTip.position.set(-0.052 * s, 0.001, -0.04);
      ...
      fistGroup.add(box(0.078, 0.036, 0.07, matSkin, 0, 0.004, 0.002));
      // Knuckle ridge (front face)
      fistGroup.add(box(0.076, 0.012, 0.014, matSkin, 0, 0.022, -0.04));
      // Knuckle bumps × 4
      for (let k = 0; k < 4; k++)
        fistGroup.add(
          cyl(0.009, 0.009, 0.011, 8, matSkin, (-0.028 + k * 0.019) * s, 0.029, -0.04, 0, 0, 0),
        );
      // Curled finger pads (front-facing strip)
      fistGroup.add(box(0.074, 0.012, 0.018, matSkinDark, 0, 0.014, -0.048));
      // Fingernail strips
      for (let k = 0; k < 4; k++)
        fistGroup.add(box(0.012, 0.003, 0.014, matNail, (-0.028 + k * 0.019) * s, 0.026, -0.042));
      // Thumb tucked along side
      fistGroup.add(
        cyl(0.01, 0.012, 0.034, 8, matSkin, -0.046 * s, 0.004, -0.016, Math.PI / 2, 0, 0.3 * s),
      );
      h.add(fistGroup);
    ...
    rightHand.rotation.set(0.1, 0.1, 0.06);
    g.add(rightHand);

    const leftHand = buildHand(-1);
    leftHand.position.set(-0.095, -0.02, 0.03);
    leftHand.rotation.set(0.08, -0.1, -0.06);
    g.add(leftHand);
