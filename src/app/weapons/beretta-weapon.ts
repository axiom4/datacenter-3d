...
export class BerettaWeapon {
  static build(): THREE.Group {
    ...
    g.add(box(0.03, 0.09, 0.006, matFrame, 0, -0.05, 0.07));

    // Trigger guard
    g.add(box(0.034, 0.006, 0.038, matFrame, 0, -0.012, -0.022));
    g.add(box(0.034, 0.02, 0.005, matFrame, 0, -0.022, -0.042));
    for (let i = 0; i < 4; i++)
      g.add(box(0.036, 0.003, 0.003, matSerr, 0, -0.013 - i * 0.004, -0.044));

    // Picatinny rail
    g.add(box(0.02, 0.005, 0.05, matRail, 0, -0.012, -0.05));

    // Slide (open-top design)
    g.add(box(0.034, 0.005, 0.19, matSlide, 0, 0.008, -0.035));
    g.add(box(0.004, 0.026, 0.19, matSlide, -0.015, 0.023, -0.035));
    g.add(box(0.004, 0.026, 0.19, matSlide, 0.015, 0.023, -0.035));
    g.add(box(0.034, 0.03, 0.022, matSlide, 0, 0.021, -0.119));
    g.add(box(0.034, 0.03, 0.057, matSlide, 0, 0.021, 0.0315));
    ...
    // Barrel
    const brlMesh = new THREE.Mesh(new THREE.CylinderGeometry(0.009, 0.01, 0.145, 20), matBrl);
    brlMesh.rotation.x = Math.PI / 2;
    brlMesh.position.set(0, 0.019, -0.058);
    g.add(brlMesh);

    const chamberMesh = new THREE.Mesh(new THREE.CylinderGeometry(0.013, 0.013, 0.02, 18), matBrl);
    chamberMesh.rotation.x = Math.PI / 2;
    chamberMesh.position.set(0, 0.019, 0.01);
    g.add(chamberMesh);
    ...
    // Locking block
    g.add(box(0.028, 0.008, 0.018, matFrame, 0, 0.006, -0.01));

    // Hammer
    g.add(cyl(0.003, 0.003, 0.04, 8, matFrame, 0, 0.016, 0.042, 0, Math.PI / 2));
    const hamMesh = new THREE.Mesh(new THREE.BoxGeometry(0.018, 0.02, 0.01), matSlide);
    hamMesh.rotation.x = 0.28;
    hamMesh.position.set(0, 0.028, 0.046);
    g.add(hamMesh);
    g.add(cyl(0.009, 0.009, 0.04, 14, matFrame, 0, 0.022, 0.048, 0, Math.PI / 2));

    // Ambidextrous safety / decocker
    for (const sx of [-0.019, 0.019]) {
      g.add(box(0.005, 0.012, 0.024, matSlide, sx, 0.02, 0.02));
      g.add(
        box(
          0.005,
          0.007,
    ...
    g.add(box(0.005, 0.01, 0.026, matFrame, -0.021, -0.001, -0.01));
    ...
    for (const sx of [-0.019, 0.019]) g.add(box(0.004, 0.085, 0.058, matGrip, sx, -0.054, 0.03));
    g.add(
      box(
        0.026,
        0.072,
    ...
    // Mag base pad
    g.add(box(0.03, 0.009, 0.06, matFrame, 0, -0.105, 0.03));

    // 3-dot sights
    g.add(box(0.005, 0.009, 0.004, matSlide, 0, 0.038, -0.122));
    g.add(cyl(0.0022, 0.0022, 0.003, 8, matWhiteDot, 0, 0.04, -0.121, Math.PI / 2));
    g.add(box(0.026, 0.007, 0.01, matSlide, 0, 0.038, 0.058));
    g.add(
      box(
        0.007,
        0.006,
    ...
    g.add(cyl(0.0022, 0.0022, 0.003, 8, matWhiteDot, -0.009, 0.04, 0.063, Math.PI / 2));
    g.add(cyl(0.0022, 0.0022, 0.003, 8, matWhiteDot, 0.009, 0.04, 0.063, Math.PI / 2));
