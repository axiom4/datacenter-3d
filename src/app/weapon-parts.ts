...
export class WeaponParts {
  ...
  static buildEOTech(): THREE.Group {
    ...
    // Front hood / shroud
    s.add(box(0.037, 0.032, 0.02, WeaponParts.matEOTech, 0, 0.002, -0.055)); // top shroud
    s.add(box(0.037, 0.006, 0.02, WeaponParts.matEOTech, 0, -0.017, -0.055)); // bottom lip
    s.add(box(0.006, 0.032, 0.02, WeaponParts.matEOTech, 0.0155, 0.002, -0.055)); // L wall
    s.add(box(0.006, 0.032, 0.02, WeaponParts.matEOTech, -0.0155, 0.002, -0.055)); // R wall

    // Holographic window (35 mm × 24 mm)
    s.add(box(0.035, 0.024, 0.002, WeaponParts.matHoloGlass, 0, 0.001, -0.022)); // front glass
    s.add(box(0.035, 0.024, 0.002, WeaponParts.matHoloCoat, 0, 0.001, -0.02)); // reticle film front
    ...
    // Window frame
    s.add(box(0.041, 0.004, 0.04, matReceiver, 0, 0.015, -0.002)); // top bar
    s.add(box(0.041, 0.004, 0.04, matReceiver, 0, -0.013, -0.002)); // bottom bar
    s.add(box(0.004, 0.028, 0.04, matReceiver, 0.0205, 0.001, -0.002)); // left
    s.add(box(0.004, 0.028, 0.04, matReceiver, -0.0205, 0.001, -0.002)); // right
    ...
    s.add(cyl(0.005, 0.005, 0.015, 10, matMuzzle, 0.03, -0.001, -0.028, 0, 0, Math.PI / 2));

    // Picatinny QD mount (1-piece)
    s.add(box(0.044, 0.012, 0.088, matMetal, 0, -0.02, 0.0)); // base plate
    s.add(box(0.044, 0.016, 0.014, matMetal, 0, -0.022, -0.04)); // front clamp
    ...
    for (let i = 0; i < 4; i++)
      s.add(box(0.042, 0.005, 0.01, matReceiver, 0, -0.03, -0.033 + i * 0.022));
  ...
  static buildACOG(): THREE.Group {
    ...
    s.add(box(0.04, 0.038, 0.092, WeaponParts.matACOG, 0, 0, 0));
    s.add(box(0.038, 0.006, 0.04, WeaponParts.matACOG, 0, -0.022, -0.026)); // taper

    // Front objective (32 mm)
    s.add(cyl(0.018, 0.02, 0.006, 14, WeaponParts.matACOG, 0, 0, -0.049, Math.PI / 2));
    ...
    s.add(cyl(0.019, 0.019, 0.01, 14, WeaponParts.matACOGRubber, 0, 0, 0.057, Math.PI / 2));
    ...
    s.add(box(0.01, 0.008, 0.076, WeaponParts.matACOG, 0, 0.023, 0.0));
    s.add(cyl(0.002, 0.002, 0.06, 8, WeaponParts.matFiberGreen, 0, 0.027, 0.0, Math.PI / 2));

    // BDC elevation strip
    s.add(box(0.008, 0.005, 0.05, matMuzzle, 0, 0.022, -0.005));

    // LaRue LT100 QD mount
    s.add(box(0.044, 0.01, 0.084, matMetal, 0, -0.024, 0.0));
    s.add(box(0.044, 0.016, 0.012, matMetal, 0, -0.026, -0.042));
    s.add(box(0.028, 0.01, 0.012, matReceiver, 0, -0.034, -0.042));
    s.add(box(0.044, 0.016, 0.012, matMetal, 0, -0.026, 0.042));
    s.add(box(0.028, 0.01, 0.012, matReceiver, 0, -0.034, 0.042));
  ...
  static buildSuppressor(): THREE.Group {
    ...
    for (const kz of [-0.055, 0.0, 0.055])
    ...
    s.add(cyl(0.02, 0.0215, 0.006, 24, matMetal, 0, 0, 0.097, Math.PI / 2));
  ...
  static buildPEQ15(): THREE.Group {
    ...
    p.add(cyl(0.002, 0.002, 0.003, 8, WeaponParts.matLaserRed, 0.009, 0.004, -0.04, Math.PI / 2));

    p.add(cyl(0.012, 0.012, 0.008, 6, matMetal, 0, 0, 0.04, Math.PI / 2));
    p.add(cyl(0.006, 0.006, 0.008, 12, matMuzzle, 0.02, 0.004, 0.01, 0, 0, Math.PI / 2));
    p.add(cyl(0.004, 0.004, 0.01, 8, matMetal, 0, 0.017, -0.01, 0));
    p.add(cyl(0.004, 0.004, 0.01, 8, matMetal, 0.02, 0.004, -0.01, 0, 0, Math.PI / 2));

    p.add(box(0.036, 0.008, 0.068, matMetal, 0, -0.017, 0));
    for (let i = 0; i < 3; i++)
      p.add(box(0.034, 0.004, 0.01, matReceiver, 0, -0.023, -0.022 + i * 0.022));
  ...
  static buildScoutLight(): THREE.Group {
    ...
    for (const kz of [-0.03, -0.015, 0.005, 0.018])
      l.add(cyl(0.0125, 0.0125, 0.004, 20, matReceiver, 0, 0, kz, Math.PI / 2));

    l.add(cyl(0.007, 0.007, 0.006, 12, WeaponParts.matACOGRubber, 0, 0, 0.05, Math.PI / 2));
    l.add(box(0.032, 0.01, 0.054, matMetal, 0, -0.023, 0));
    l.add(box(0.028, 0.005, 0.05, matReceiver, 0, -0.03, 0));
    l.add(
      cyl(
        0.003,
        0.003,
  ...
  static buildVFG(): THREE.Group {
    ...
    v.add(box(0.032, 0.01, 0.04, matMetal, 0, 0.005, 0));
    v.add(box(0.028, 0.005, 0.036, matReceiver, 0, 0.001, 0));
    v.add(cyl(0.003, 0.003, 0.03, 8, matMetal, 0.018, 0.01, 0, 0, 0, Math.PI / 2));
  ...
  static buildKACRail(length: number): THREE.Group {
    ...
    for (const { ox, oy, rz } of faceData) {
      const face = new THREE.Mesh(new THREE.BoxGeometry(0.042, 0.004, length), matMetal);
      face.position.set(ox, oy, 0);
      face.rotation.z = rz;
      r.add(face);

      const teeth = Math.floor(length / 0.02);
      for (let t = 0; t < teeth; t++) {
        const tz = -length / 2 + 0.008 + t * 0.02;
        const tooth = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.008, 0.01), matReceiver);
    ...
    const walls = Math.ceil(length / 0.05);
    for (let w = 0; w <= walls; w++) {
      const wz = -length / 2 + w * (length / walls);
      r.add(box(0.048, 0.048, 0.003, matMetal, 0, 0, wz));
    }

    r.add(cyl(0.02, 0.02, length, 20, matReceiver, 0, 0, 0, Math.PI / 2));
...
export const buildEOTech = (...args: Parameters<typeof WeaponParts.buildEOTech>) =>
  WeaponParts.buildEOTech(...args);
export const buildACOG = (...args: Parameters<typeof WeaponParts.buildACOG>) =>
  WeaponParts.buildACOG(...args);
export const buildSuppressor = (...args: Parameters<typeof WeaponParts.buildSuppressor>) =>
  WeaponParts.buildSuppressor(...args);
export const buildPEQ15 = (...args: Parameters<typeof WeaponParts.buildPEQ15>) =>
  WeaponParts.buildPEQ15(...args);
export const buildScoutLight = (...args: Parameters<typeof WeaponParts.buildScoutLight>) =>
  WeaponParts.buildScoutLight(...args);
export const buildVFG = (...args: Parameters<typeof WeaponParts.buildVFG>) =>
  WeaponParts.buildVFG(...args);
export const buildKACRail = (...args: Parameters<typeof WeaponParts.buildKACRail>) =>
  WeaponParts.buildKACRail(...args);
